...
The goal of GCSM is to implement the generic composite similarity measure (GCSM), described in "A generic composite measure of similarity between geospatial variables" by Yadong Liu, Kwang Soo Kim, Robert M. Beresford and David H. Fleisherd (2020) <doi:>. This package also provides implements of other composite measures, SSIM and CMSC. Functions are given to compute the composite similarity between vectors (e.g, `gcsm`), on spatial windows (e.g., `gcsm_sw`) or temporal windows (e.g., `gcsm_tw`).
